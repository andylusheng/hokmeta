...
import { arcanaCatalog, getLocalizedRuneDescription, getLocalizedRuneName } from '@/lib/arcana-catalog';
...
export function ArcanaPageView({ locale = 'en' }: { locale?: Locale }) {
  ...
  return (
    <div className="container-wide">
      ...
      <p className="mb-2 text-gray-400">{t('arcana.subtitle', { count: runes.length })}</p>
        ...
        {runes.map((rune) => (
          <div key={rune.id} className="card-hover flex gap-3">
            ...
            <div className="min-w-0 flex-1">
              <h2 className="font-bold text-white">{getLocalizedRuneName(rune, locale)}</h2>
              {rune.level != null && (
              ...
              {(rune.usedByCount ?? 0) > 0 && (
                <p className="mt-2 text-xs text-gray-500">
                  {t('arcana.usedBy')}:{' '}
                  <span className="font-semibold text-hok-gold">{rune.usedByCount}</span>
                </p>
              )}
            </div>
          </div>
