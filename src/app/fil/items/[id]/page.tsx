...
export function generateMetadata({ params }: { params: { id: string } }) {
  ...
  return buildMetadata({
    title: `${getLocalizedItemName(item, 'fil')} — HOKMeta Items`,
    description: getLocalizedItemDescription(item, 'fil'),
    path: `/fil/items/${item.id}`,
    locale: 'fil',
  });
}
...
export default function ItemDetailZhPage({ params }: { params: { id: string } }) {
  const item = getItemById(params.id);
  if (!item) notFound();
  return <ItemDetailPageView item={item} locale="fil" />;
}
