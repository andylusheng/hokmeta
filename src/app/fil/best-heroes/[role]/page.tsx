...
export function generateMetadata({
  ...
  return buildMetadata({
    title: defaultTitle(t('bestHeroes.roleTitle', { role: translateRole(role, 'fil') })),
    description: t('bestHeroes.roleSubtitle', { count: 0, role: translateRole(role, 'fil') }),
...
export default function ZhTWBestHeroesRolePage({
  ...
  const role = roleMap[params.role.toLowerCase()];
  if (!role) notFound();
  return <BestHeroesRoleView role={role} locale="fil" />;
}
